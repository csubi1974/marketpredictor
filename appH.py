...
def load_data(ticker, start_date, end_date):
  try:
      ...
      datos = stock.history(start=start_date, end=end_date + timedelta(days=1), auto_adjust=False)
...
def generate_results(model, data):
  ...
  data['Pronóstico'] = np.where(data['Prediction'] == 1, 'Subida', 'Bajada')
  data['Real'] = data['Target']
  data['Resultado'] = np.where(data['Prediction'] == data['Real'], 'Correcto', 'Incorrecto')
  results = data[['Open', 'Close', 'Pronóstico', 'Resultado']]
  return results

# Opción de descarga de CSV con delimitador ';'
def download_csv(dataframe, ticker):
  csv = dataframe.to_csv(index=True, sep=';')
...
def main():
  ...
  # Agregar la tabla de los últimos 100 días
  if model:
      st.subheader('Últimos 100 días de negociación')
      results_df = generate_results(model, data)
      
      # Filtrar para obtener solo los últimos 100 días
      last_100_days = results_df.tail(100)
      
      # Formatear la tabla
      formatted_table = last_100_days.reset_index()
      formatted_table = formatted_table.rename(columns={
          formatted_table.columns[0]: 'Fecha',  # Asumimos que la primera columna es la fecha
          'Open': 'Apertura',
          'Close': 'Cierre',
      ...
      formatted_table['Fecha'] = pd.to_datetime(formatted_table['Fecha'])
      
      formatted_table['Fecha'] = formatted_table['Fecha'].dt.strftime('%Y-%m-%d')
      formatted_table['Apertura'] = formatted_table['Apertura'].round(2)
      formatted_table['Cierre'] = formatted_table['Cierre'].round(2)

      # Mostrar el gráfico de velas japonesas para la fecha seleccionada
      selected_date = st.selectbox("Selecciona una fecha para ver el gráfico de velas:", formatted_table['Fecha'].tolist())
      
      if selected_date:
          selected_date = pd.to_datetime(selected_date)
          candlestick_fig = plot_candlestick_chart(data, selected_date)
      ...
      correct_count = formatted_table['Resultado'].value_counts().get('Correcto', 0)
      incorrect_count = formatted_table['Resultado'].value_counts().get('Incorrecto', 0)
      st.write(f"**Resumen de los últimos 100 días:**")
      st.write(f"Correctos: {correct_count}")
      st.write(f"Incorrectos: {incorrect_count}")
