...
def perform_backtesting(model, data, days=60):
    features = ['Returns', 'MA5', 'MA20', 'MA50', 'RSI', 'Volatility', 'Volume_Change']
    test_data = data.iloc[-days:]  # Incluimos el último día
...
def main():
    ...
    # Mostrar últimos precios de cierre y apertura
    ...
    if model:
        ...
        if len(data) >= 60:
            ...
            # Visualización del backtesting con Plotly
            bt_df = pd.DataFrame({
                'Fecha': data.index[-60:],  # Incluir el último día
                'Predicción': predictions,
                'Valor Real': actual_values
            })

            # Asignar color a las barras
            ...
            fig.add_trace(go.Bar(
                ...
                text=['Subida' if p == 1 else 'Bajada' for p in bt_df['Predicción']],
                textposition='auto'
            ))
            ...
            fig.update_layout(
                ...
                height=min(500, len(bt_df) * 20)  # Ajuste dinámico del tamaño
            )

            st.plotly_chart(fig)

            # Calcular porcentaje de predicciones correctas
